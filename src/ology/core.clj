...
(defn scatter-file
  ...
  (let [log-file-reader (clojure.java.io/reader (clojure.java.io/file input-file-path))
        parsed-lines (remove nil? (map parse-line (line-seq log-file-reader)))
        date-partitions (partition-by first parsed-lines)]  
        ; Iterate over every date in the file.
        (doseq [date-partition date-partitions]
          (let [partition-date (first (first date-partition))
                filename (format/unparse scatter-file-date-formatter partition-date)
                output-file (clojure.java.io/writer (str dir-base "/" filename) :append true)]
                ; Throw away date.
                (info "Writing for date " partition-date)
                (doseq [[_ doi domain] date-partition]
                  (.write output-file (str (pr-str [doi domain]) "\n")))
                (info "Done writing for date " partition-date)
                (.close output-file)))))
